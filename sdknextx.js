let wasm_bindgen;
(()=>{var R=(()=>{let p=Object.getPrototypeOf,I,j,m,T,S={isConnected:1},x=1e3,P,E={},w=p(S),N=p(p),b,V=(a,u,y,s)=>(a??(setTimeout(y,s),new Set)).add(u),O=(a,u,y)=>{let s=m;m=u;try{return a(y)}catch(h){return console.error(h),y}finally{m=s}},k=a=>a.filter(u=>u._dom?.isConnected),D=a=>P=V(P,a,()=>{for(let u of P)u._bindings=k(u._bindings),u._listeners=k(u._listeners);P=b},x),F={get val(){return m?._getters?.add(this),this.rawVal},get oldVal(){return m?._getters?.add(this),this._oldVal},set val(a){m?._setters?.add(this),a!==this.rawVal&&(this.rawVal=a,this._bindings.length+this._listeners.length?(j?.add(this),I=V(I,this,M)):this._oldVal=a)}},d=a=>Object.create(F,{rawVal:{value:a,writable:!0,enumerable:!0,configurable:!0},_oldVal:{value:a,writable:!0,enumerable:!0,configurable:!0},_bindings:{value:[],writable:!0,enumerable:!0,configurable:!0},_listeners:{value:[],writable:!0,enumerable:!0,configurable:!0}}),t=(a,u)=>{let y={_getters:new Set,_setters:new Set},s={f:a},h=T;T=[];let v=O(a,y,u);v=(v??document).nodeType?v:new Text(v);for(let C of y._getters)y._setters.has(C)||(D(C),C._bindings.push(s));for(let C of T)C._dom=v;return T=h,s._dom=v},e=(a,u=d(),y)=>{let s={_getters:new Set,_setters:new Set},h={f:a,s:u};h._dom=y??T?.push(h)??S,u.val=O(a,s,u.rawVal);for(let v of s._getters)s._setters.has(v)||(D(v),v._listeners.push(h));return u},i=(a,...u)=>{for(let y of u.flat(1/0)){let s=p(y??0),h=s===F?t(()=>y.val):s===N?t(y):y;h!=b&&a.append(h)}return a},r=(a,u,...y)=>{let[s,...h]=p(y[0]??0)===w?y:[{},...y],v=a?document.createElementNS(a,u):document.createElement(u);for(let[C,J]of Object.entries(s)){let l=_=>_?Object.getOwnPropertyDescriptor(_,C)??l(p(_)):b,n=u+","+C,c=E[n]??(E[n]=l(p(v))?.set??0),A=C.startsWith("on")?(_,L)=>{let G=C.slice(2);typeof L=="function"&&v.removeEventListener(G,L)(typeof _=="function")&&v.addEventListener(G,_)}:c?c.bind(v):v.setAttribute.bind(v,C),$=p(J??0);C.startsWith("on")||$===N&&(J=e(J),$=F),$===F?t(()=>(A(J.val,J._oldVal),v)):A(J)}return i(v,...h)},o=a=>({get:(u,y)=>r.bind(b,a,y)}),f=new Proxy(a=>new Proxy(r,o(a)),o()),g=(a,u)=>u?u!==a&&a.replaceWith(u):a.remove(),M=()=>{let a=0,u=[...I].filter(s=>s.rawVal!==s._oldVal);do{j=new Set;for(let s of new Set(u.flatMap(h=>h._listeners=k(h._listeners))))e(s.f,s.s,s._dom),s._dom=b}while(++a<100&&(u=[...j]).length);let y=[...I].filter(s=>s.rawVal!==s._oldVal);I=b;for(let s of new Set(y.flatMap(h=>h._bindings=k(h._bindings))))g(s._dom,t(s.f,s._dom)),s._dom=b;for(let s of y)s._oldVal=s.rawVal};return{add:i,tags:f,state:d,derive:e,hydrate:(a,u)=>g(a,t(u,a))}})();var se=p=>{let{fromEntries:I,entries:j,keys:m}=Object,{get:T,set:S,deleteProperty:x,ownKeys:P}=Reflect,{state:E,derive:w,add:N}=p,b,V=1e3,O,k,D=Symbol(),F=Symbol(),d=Symbol(),t=Symbol(),e=Symbol(),i=Symbol(),r=l=>(l[d]=1,l),o=l=>{if(l?.[d]){let n=E();return w(()=>{let c=l();n.rawVal instanceof Object&&c instanceof Object?C(n.rawVal,c):n.val=f(c)}),n}else return E(f(l))},f=l=>!(l instanceof Object)||l[F]?l:new Proxy((l[D]=l,l[F]=I(j(l).map(([n,c])=>[n,o(c)])),l[t]=[],l[e]=E(1),l),{get:(n,c,A)=>n[F].hasOwnProperty(c)?n[F][c].val:(c==="length"&&n[e].val,T(n,c,A)),set(n,c,A,$){let _=n[F];if(_.hasOwnProperty(c))return _[c].val=f(A),1;let L=c in n,G=L&&c==="length"&&A!==n.length;if(S(n,c,A))return L?G&&++n[e].val:S(_,c,o(A))&&(++n[e].val,u($,c,_[c])),1},deleteProperty:(n,c)=>(x(n[F],c)&&y(n,c),x(n,c)&&++n[e].val),ownKeys:n=>(n[e].val,P(n))}),g=l=>l[F],M=l=>l[F]?new Proxy(l[F],{get:(n,c)=>M(n[c].rawVal)}):l,z=l=>l[t]=l[t].filter(n=>n._containerDom.isConnected),a=(l,n,c,A,{_containerDom:$,f:_})=>{let L=Array.isArray(l);if(N($,()=>$[i][n]=_(c,()=>delete l[n],L?Number(n):n)),L&&!A&&n!=l.length-1){let G=Number(n);$.insertBefore($.lastChild,$[i][m(l).find(W=>Number(W)>G)])}},u=(l,n,c)=>z(l).forEach(a.bind(O,l,n,c,k)),y=(l,n)=>{for(let c of z(l)){let A=c._containerDom[i];A[n]?.remove(),delete A[n]}},s=l=>(b??(b=(setTimeout(()=>(b.forEach(z),b=O),V),new Set))).add(l),h=(l,n,c)=>{let A={_containerDom:l instanceof Function?l():l,f:c};A._containerDom[i]={},n[t].push(A),s(n);for(let[$,_]of j(n[F]))a(n,$,_,1,A);return A._containerDom},v=(l,n)=>{for(let[$,_]of j(n)){let L=l[$];L instanceof Object&&_ instanceof Object?v(L,_):l[$]=_}for(let $ in l)n.hasOwnProperty($)||delete l[$];let c=m(n),A=Array.isArray(l);if(A||m(l).some(($,_)=>$!==c[_])){if(A)l.length=n.length;else{++l[e].val;let $=l[D],_={...$};for(let L of c)delete $[L];for(let L of c)$[L]=_[L]}for(let{_containerDom:$}of z(l)){let{firstChild:_,[i]:L}=$;for(let G of c)_===L[G]?_=_.nextSibling:$.insertBefore(L[G],_)}}return l},C=(l,n)=>{k=1;try{return v(l,n instanceof Function?Array.isArray(l)?n(l.filter(c=>1)):I(n(j(l))):n)}finally{k=O}},J=l=>Array.isArray(l)?l.filter(n=>1).map(J):l instanceof Object?I(j(l).map(([n,c])=>[n,J(c)])):l;return{calc:r,reactive:f,stateFields:g,raw:M,list:h,replace:C,compact:J}};var ne=(()=>{function p(S){return/^[a-zA-Z_][a-zA-Z_0-9]+$/.test(S)?S:`"${S}"`}function I(S=""){let x=[];if(S.includes("{{")&&S.includes("}}")){let E=S.indexOf("{{"),w=S.indexOf("}}"),N=S.slice(0,E).trim(),b=S.slice(w+2).trim(),V=S.slice(E+2,w).trim();if(N.length>0&&x.push(JSON.stringify(N)),V.length>0&&x.push(V),b.length>0){let O=I(b).filter(k=>k);b.length>0&&x.push(...O)}}else x.push(JSON.stringify(S));let P=x.filter(E=>E);return P.length===0?[]:P}function j(S,x=new Set){if(S.nodeType===3){let e=I(S.nodeValue.trim()).filter(i=>i&&i!='""').join(",");return{tags:x,body:e}}if(!S.tagName)return{tags:x,body:""};let P=S.tagName.toLowerCase();x.add(P);let E=[],w={};Array.from(S.attributes).forEach(t=>{w[t.name.trim()]=t.value.trim()}),w.hasOwnProperty("x-tags")&&(w["x-tags"].split(/[\s|,]+/).map(e=>e.trim()).filter(e=>e).forEach(e=>{x.add(e.toLowerCase())}),delete w["x-tags"]),Object.keys(w).forEach(t=>{t.startsWith("x-script")&&(E.push(w[t]),delete w[t])});let N="";w["x-list"]&&(N=w["x-list"]||"",delete w["x-list"]);let b=Object.keys(w).map(t=>{if(t.startsWith("x-")){let e=t.substring(2);return`${p(e)}:${w[t]||""}`}else{let e=t;return e.startsWith("on")?`${p(e)}:${w[t]||""}`:`${p(e)}:${JSON.stringify(w[t])}`}}).join(","),V=[],O="";P==="script"?O=JSON.stringify(S.textContent):(V=Array.from(S.childNodes).map(t=>j(t,x)),O=V.filter(t=>t.body).map(t=>t.body).join(","));let k=P.replace(/-/g,"_"),D="";var F=()=>{if(N){let t=N.match(/^\s*(\()?(?<k>[^\(\)]+)(\))?\s+in\s+(?<d>[a-zA-Z0-9\.\$_]+)\s*$/);if(t){let{k:e,d:i}=t.groups;D=`$list(${k},${i},(${e}) =>${O||""})`}}};Object.keys(w).length>0?(D=`${k}({${b}} ${O?","+O:""})`,F()):(D=O?`${k}(${O})`:`${k}()`,F());let d=D;return E.length>0&&(d=`()=>{ ${E.join(`
